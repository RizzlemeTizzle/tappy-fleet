...
async function proxyOrganizationUser(
  req: NextRequest,
  ...
  const backendRes = await fetch(
    `${API_URL}/api/fleet/companies/${companyId}/organization-users/${memberId}`,
    init,
  );
...
export const PUT = (
  req: NextRequest,
  ctx: { params: Promise<{ companyId: string; memberId: string }> },
) => proxyOrganizationUser(req, ctx, 'PUT');

export const DELETE = (
  req: NextRequest,
  ctx: { params: Promise<{ companyId: string; memberId: string }> },
) => proxyOrganizationUser(req, ctx, 'DELETE');
